...
module "jenkins_controller_vm" {
  ...
  virtual_machine_custom_data = join("\n", ["#cloud-config", yamlencode({
    ...
    write_files = [
      ...
      {
        content = join("\n", concat(
          [for ip in toset(module.builder_vm[*].virtual_machine_ip_address) : "ssh://remote-build@${ip} x86_64-linux /etc/secrets/remote-build-ssh-key 8 1 kvm,nixos-test,benchmark,big-parallel - -"],
